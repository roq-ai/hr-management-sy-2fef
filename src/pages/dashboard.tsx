...
import { getEmployeeBenefits } from 'apiSdk/employee-benefits';
import { EmployeeBenefitsInterface } from 'interfaces/employee-benefits';
import { getEmployeePersonalData } from 'apiSdk/employee-personal-data';
...
import { EmployeeBenefitsListPage as ShowCaseList } from 'pages/employee-benefits';

function DashboardPage() {
  const { session } = useSession();

  const employeeBenefitsStat = useSWR<PaginatedInterface<EmployeeBenefitsInterface>>(
    () => `/employee-benefits`,
    () => getEmployeeBenefits({ limit: 1, offset: 0 }),
  );

  const employeePersonalDataStat = useSWR<PaginatedInterface<EmployeePersonalDataInterface>>(
    () => `/employee-personal-data`,
    () => getEmployeePersonalData({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: employeeBenefitsStat?.data?.totalCount,
      isLoading: employeeBenefitsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Employee Personal Data',
      result: employeePersonalDataStat?.data?.totalCount,
      isLoading: employeePersonalDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Payrolls',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Employee Benefits
                </Text>
                <NextLink href={`employee-benefits`} passHref legacyBehavior>
                  <Button
